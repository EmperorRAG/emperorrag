...
const program = Effect.gen(function* (_) {
  ...
  if (process.argv[2]) {
    ...
    yield* _(
      Console.log(
        "No trace file provided. Attempting to find all trace files from the latest VS Code session...",
      ),
    );
  ...
  if (logFilePaths.length === 0) {
    yield* _(
      Console.error(
        "Please provide a valid path to trace.json or ensure VS Code logs exist.",
      ),
    );
    yield* _(
      Console.error(
        "Usage: npx ts-node packages/tsserver-analyzer/src/index.ts [path-to-trace]",
      ),
    );
    return;
  }
  ...
  yield* _(
    Console.log(
      `Loaded ${mappedPaths.length} path mappings from tsconfig.base.json`,
    ),
  );
  ...
  yield* _(
    Effect.gen(function* (__) {
      const stats = yield* __(
        TSServerAnalyzerService.runAnalysis(logFilePaths),
      );
