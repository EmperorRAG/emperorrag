...
export class EmailAuthServerInputError extends Schema.TaggedError<EmailAuthServerInputError>()('EmailAuthServerInputError', {
	...
	pipe(
		...
		Schema.decode(
			Schema.transformOrFail(Schema.Unknown, SignUpEmailAuthServerParams, {
				...
				encode: (output) => Schema.encode(SignUpEmailAuthServerParams)(output),
			})
		),
