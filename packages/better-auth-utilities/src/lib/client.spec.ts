...
import type { AuthClientFor } from './client.types.ts';
...
describe('better-auth-utilities: client', () => {
	...
	describe('Plugin Initialization', () => {
		...
		it('should handle advanced client plugins', () => {
			const config = defineConfig({
				enabledClientPlugins: ['multiSession', 'anonymous', 'phoneNumber', 'emailOTP', 'lastLoginMethod', 'oneTimeToken'] as const,
				server: {
					secret: 'test-secret-key-minimum-32-chars-long',
				},
				client: {},
	...
	describe('Error Handling', () => {
		...
		it('should handle plugin initialization errors gracefully', () => {
			...
			// Should not throw
			expect(() => createAuthClient(config)).not.toThrow();

			expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to initialize client plugin twoFactor:', expect.any(Error));
		});
		...
		it('should handle errors from plugins requiring additional packages', () => {
			...
			expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to initialize client plugin passkey:', expect.any(Error));
		});
	});
	...
	describe('Integration Tests', () => {
		it('should create a complete production-ready client', () => {
			...
			const config = defineConfig({
				enabledClientPlugins: ['username', 'twoFactor', 'admin', 'organization', 'apiKey', 'anonymous'] as const,
				server: {
	...
	describe('Type Inference', () => {
		it('should infer client type correctly', () => {
			...
			const client = createAuthClient(config);
			type Client = AuthClientFor<typeof client>;
	...
	describe('Edge Cases', () => {
		...
		it('should preserve plugin order', () => {
			...
			const calls = [mockClientPlugins.usernameClient, mockClientPlugins.twoFactorClient, mockClientPlugins.adminClient];
