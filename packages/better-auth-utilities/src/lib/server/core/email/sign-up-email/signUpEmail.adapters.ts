...
export const ensureRecord =
	(message: string) =>
	(u: unknown): Effect.Effect<Readonly<Record<string, unknown>>, EmailAuthServerInputError> => {
...
// 2. getPropE
export const getPropE =
	(key: string) =>
	(record: Readonly<Record<string, unknown>>): Effect.Effect<unknown, never> => {
...
export const requireDefined =
	(message: string) =>
	(u: unknown): Effect.Effect<unknown, EmailAuthServerInputError> => {
		if (u === undefined) {
			return Effect.fail(new EmailAuthServerInputError({ message }));
		}
...
export const requireBoolean =
	(message: string) =>
	(u: unknown): Effect.Effect<boolean, EmailAuthServerInputError> => {
		if (typeof u === 'boolean') {
			return Effect.succeed(u);
		}
		return Effect.fail(new EmailAuthServerInputError({ message }));
...
export const requireHeaders =
	(message: string) =>
	(u: unknown): Effect.Effect<Headers, EmailAuthServerInputError> => {
...
export const extractSignUpCtxStrict = (rawParams: unknown): Effect.Effect<SignUpTransportCtx, EmailAuthServerInputError> =>
	pipe(
		rawParams,
		ensureRecord('rawParams must be an object'),
		Effect.flatMap((params) =>
			Effect.gen(function* () {
				const headers = yield* pipe(params, getPropE('headers'), Effect.flatMap(optionalOf(requireHeaders('headers must be Headers'))));

				const asResponse = yield* pipe(params, getPropE('asResponse'), Effect.flatMap(optionalOf(requireBoolean('asResponse must be boolean'))));
