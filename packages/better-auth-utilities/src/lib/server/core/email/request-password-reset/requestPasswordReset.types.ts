...
export type AuthServerApiRequestPasswordResetPropsFor<
...
export type AuthServerApiRequestPasswordResetParamsFor<
  T extends AuthServerFor = AuthServerFor,
> = Parameters<AuthServerApiRequestPasswordResetPropsFor<T>>[0];

export type AuthServerApiRequestPasswordResetResultFor<
  T extends AuthServerFor = AuthServerFor,
> = ReturnType<AuthServerApiRequestPasswordResetPropsFor<T>>;

export interface RequestPasswordResetPropsFor<
  T extends AuthServerFor = AuthServerFor,
> {
  (
    params: AuthServerApiRequestPasswordResetParamsFor<T>,
  ): Effect.Effect<
    Awaited<AuthServerApiRequestPasswordResetResultFor<T>>,
    AuthServerError,
    T
  >;
}
