...
import type {
  AuthServerApiForgetPasswordCallbackParamsFor,
  forgetPasswordCallbackPropsFor,
...
export const forgetPasswordCallbackServerService: forgetPasswordCallbackPropsFor = (
  ...
  Effect.flatMap(
    AuthServerTag,
    (authServer) =>
      Effect.tryPromise(() => authServer.api.forgetPasswordCallback(params)).pipe(Effect.catchAll(mapApiError)),
  );
