 ...
 */
const loadEnvironmentFile = (): Effect.Effect<
  void,
        ...
        pipe(
          Effect.tryPromise({
            try: () => fs.readFile(ENV_FILE_PATH, "utf-8"),
              ...
              new EnvironmentLoadFailure(
                "Failed to load Better Auth environment file.",
                error,
              ),
          }),
...
const getSeedAdminUsersConfig = (): SeedAdminUsersConfig =>
  pipe(
    ...
    ReadonlyArray.reduce(
      {
        seeds: [] as ReadonlyArray<SeedAdminUser>,
        missingPrefixes: [] as ReadonlyArray<string>,
      },
 ...
 */
const createCredentialId = (seed: SeedAdminUser): string =>
  `${seed.id}-credential`;
...
const hashAdminPassword = (
  ...
  Effect.tryPromise({
    try: () => hashPassword(password),
      ...
      new PasswordHashFailure("Failed to hash admin user password.", error),
  });
...
const ensureAdminUser = (
  ...
  pipe(
    ...
    Effect.flatMap((hashedPassword) =>
      persistAdminUser(prisma, seed, hashedPassword),
    ),
    Effect.tap(() =>
      Effect.sync(() =>
        console.log(
          `[better-auth] Seeded administrator account for ${seed.email}.`,
        ),
      ),
    ),
  );
      ...
      Effect.flatMap(() =>
        Match.value(config.seeds.length).pipe(
          Match.when(0, () =>
            Effect.sync(() =>
              console.log(
                "[better-auth] No admin seed users configured; skipping seeding workflow.",
              ),
            ),
          ),
          Match.orElse(() =>
            pipe(
              ...
              Effect.tap(() =>
                Effect.sync(() =>
                  console.log(
                    "[better-auth] Completed admin user seeding workflow.",
                  ),
                ),
              ),
            ),
...
void Effect.runPromise(mainProgram).then(
  () => {
    console.log(
      "[better-auth] Admin on-startup script completed successfully.",
    );
  },
  (error) => {
