...
- `EmailAuthServerDeps` structure providing a preloaded `AuthServer` instance, transports, rate limiters, and telemetry hooks consumed by curried service helpers `(deps) => (input) => result`.
- `PreloadedEmailAuthService` collecting every handler after dependency injection so controllers can reuse a single dependency resolution call.
